...
const BuyTicket = () => {
  ...
  const RenderInfo = () => {
    ...
    return (
      <VStack justify={"space-around"} align="center">
        <HStack spacing={5} mb={"1rem"} border={"1px solid black"} px={5}>
          <Text>{info.name}</Text>
          <Text>{info.time}</Text>
      ...
      {selectedMovie.map((movie) => (
        ...
        >
          ...
          >
            ...
            <HStack align={"center"} justify={"center"} w={"100%"} h={"30px"}>
              <Select
                w="35%"
              ...
              >
                {movie.movie_session.city.map((city, index) => (
                  <option key={index} w="200px" value={JSON.stringify(city)}>
                    {city.name}
                  </option>
                ))}
              </Select>
            ...
            {isSelected ? (
              ...
              <Box mt="30px">
                <Text textAlign={"center"}>
                  Escolha onde quer assistir o filme
                </Text>
              </Box>
            )}
            {toRender && (
              <VStack w={"90%"} ml="1.7rem" mt={"1rem"} spacing={5} py={3}>
